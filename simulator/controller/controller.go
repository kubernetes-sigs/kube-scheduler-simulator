...
func NewController(client clientset.Interface, c *restclient.Config) (func(), error) {
	...
	err := mustSetupScheduler(client, ctx.Done())
	if err != nil {
		cancel()
...
func mustSetupScheduler(client clientset.Interface, stopCh <-chan struct{}) error {
	err := StartPersistentVolumeController(client, stopCh)
	if err != nil {
